      ...
      - { magic.
          ...
          all:match goal with
              ...
              | |- eqterm (ctxextend ?G (Subst ?A ?sbs))
                         (subst ?u (sbcomp ?sbs (sbweak ?G (Subst ?A ?sbs))))
                         (subst _ (sbshift ?G ?A ?sbs))
                         _ =>
                instantiate (1 := subst u (sbweak D A))
              | |- eqterm ?D (subst _ (sbzero ?D ?A ?u)) ?u _ =>
                instantiate (1 := var 0)
              | |- eqterm (ctxextend ?G (Subst ?A ?sbs))
                         (subst _ (sbshift ?G ?A ?sbs))
                         (var 0)
                         _ =>
                instantiate (1 := var 0)
              | |- eqterm ?G (subst _ (sbzero ?G (Subst ?A ?sbs) (subst ?v ?sbs)))
                         (subst ?v ?sbs) _ =>
                instantiate (1 := var 0)
              | |- eqterm ?G (subst _ (sbzero ?G (Subst ?A ?sbs) (subst ?v ?sbs)))
                         (subst ?u ?sbs) _ =>
                instantiate (1 := subst (subst u sbs) (sbweak G (Subst A sbs)))
              | _ => idtac
              end.
          Unshelve. all:try okmagic.
          Unshelve. all:try (check_goal ; assumption).
          ...
          1: instantiate (1 := refl (Subst (Subst A sbs) (sbweak G (Subst A sbs)))
                                   (var 0)).
          1: (* magic. *) (* We need to add it to magic! *) idtac.
          Focus 1.
          pushsubst1.
          - magic.
          - magic.
