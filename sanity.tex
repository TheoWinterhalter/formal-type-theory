...
\end{equation*}
%
Then $\isctx{\G}$ and by induction hypothesis on the left premise, while
$\isctx{(\ctxextend{\D}{\A})}$ follows using the induction hypothesis on the middle
premise and $\rl{ctx-extend}$.
...
\end{equation*}
%
We apply $\rl{ty-subst}$ to the right premise and to $\rl{subst-weak}$ applied to the
right premise.
...
\meta{This in turn we don't have (same kind of trouble as $\rl{term-app}$).}
